-- Seed: 09 — rapports de salaire (SQLite)
PRAGMA foreign_keys = ON;

INSERT OR IGNORE INTO "salary_reports"
...
SELECT 1, strftime('%Y-%m', 'now'), s.name, 'Admin',
       500000.00, 80000.00, 420000.00,
       25000.00, 15000.00, 10000.00,
       370000.00, 5, 0.00,
       520.00, 500000.00, 961.54,
       '{}', 1, 0,
       'Nouvel employé inscrit', NULL, 1, datetime('now'), datetime('now')
FROM stores s
WHERE s.id = 1;
