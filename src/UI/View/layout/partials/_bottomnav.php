...
// (route_visible, partagé par AuthMiddleware, RBAC-V2) plutôt qu'une clé de
// permission recopiée à la main à côté du lien — élimine la classe de bug où
// la nav affiche un lien que la route bloquerait ensuite au clic (c'était le
// cas ici pour "messages"/"daily_reports", qui n'avaient aucun contrôle RBAC
// bien que leurs routes en exigent un).
$routeVisible = $route_visible ?? fn(string $routeName): bool => true;
...
$_bnAdminPool = [
        'shifts'        => ['icon' => 'calendar', 'label' => __('shifts'),        'href' => fn() => route_url('admin.shifts.timeline'), 'feat' => 'shifts',        'route' => 'admin.shifts.timeline'],
        'team'          => ['icon' => 'users',     'label' => __('team'),          'href' => fn() => route_url('admin.users'),           'feat' => null,            'route' => 'admin.users'],
        'requests'      => ['icon' => 'clipboard', 'label' => __('requests'),      'href' => fn() => route_url('admin.timeoff'),         'feat' => 'timeoff',       'route' => 'admin.timeoff'],
        'messages'      => ['icon' => 'message',   'label' => __('messages'),      'href' => fn() => route_url('admin.messages'),        'feat' => 'messages',      'route' => 'admin.messages'],
        'swaps'         => ['icon' => 'arrows',    'label' => __('swaps'),         'href' => fn() => route_url('admin.swap_requests'),   'feat' => 'swaps',         'route' => 'admin.swap_requests'],
        'timeclocks'    => ['icon' => 'clock',     'label' => __('timeclocks'),    'href' => fn() => route_url('admin.timeclocks'),      'feat' => 'timeclock',     'route' => 'admin.timeclocks'],
        'daily_reports' => ['icon' => 'chart',     'label' => __('daily_reports'), 'href' => fn() => route_url('admin.daily_reports.all'),  'feat' => 'daily_reports', 'route' => 'admin.daily_reports.all'],
];
$_bnEmployeePool = [
...
foreach ($_bnKeys as $_bnKey) {
    ...
    $f = $_bnItem['feat'] ?? null;
    $r = $_bnItem['route'] ?? null;
    if ($f !== null && !$feat($f)) continue;
    if ($r !== null && !$routeVisible($r)) continue;
