...
## 🧩 Modular Bundles
Features are split into **Core** (always on) and **Bundles** (`src/Bundles/*/`) — both are part of this AGPL-3.0 repository, nothing is gated behind a separate license. Any feature beyond the always-on core is expected to ship as a bundle: today that's Messaging, Daily Reports, Store Photos, and the Salary Report. Time off, shift swaps, open shifts (shift claims), and the remaining HR reports (hiring, resignation) currently live in Core rather than as bundles — they are candidates for a future migration into their own bundles. Hiring, resignation, and salary reports used to share one `AdminReportController` with an internal `repo(string $type)` dispatch; extracting salary moved the CRUD/PDF logic common to all three report types into a reusable `HasStaffReportCrud` trait, used by the bundle's `AdminSalaryReportController`. Its `calculateSalaryPreset()` reads `DailyReportRepositoryInterface` (the DailyReport bundle) to pre-fill the month's sales total — a bundle-to-bundle dependency, so salary-report creation assumes daily-report stays enabled. `AdminReportController` keeps its two-type dispatch for hiring and resignation until resignation also gets its own bundle, at which point hiring alone will switch to `HasStaffReportCrud` too.
