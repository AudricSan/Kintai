...
## 🧩 Modular Bundles
Features are split into **Core** (always on) and **Bundles** (`src/Bundles/*/`) — both are part of this AGPL-3.0 repository, nothing is gated behind a separate license. Any feature beyond the always-on core is expected to ship as a bundle: today that's Messaging, Daily Reports, Store Photos, and the Hiring Report. Time off, shift swaps, open shifts (shift claims), and the remaining HR reports (resignation, salary) currently live in Core rather than as bundles — they are candidates for a future migration into their own bundles. Hiring, resignation, and salary reports used to share one `AdminReportController` with an internal `repo(string $type)` dispatch; extracting hiring moved the CRUD/PDF logic common to all three report types into a reusable `HasStaffReportCrud` trait, used by the bundle's `AdminHiringReportController`. Unlike resignation/salary, `HiringReportRepositoryInterface` stays a Core service — `AdminUserController` reads/writes it directly to auto-generate a hiring report on every employee creation (standard form and Excel quick-create), a Core workflow that must keep working even when this bundle is disabled. Disabling "hiring-report" removes only the browse/edit/PDF UI, not that auto-generation or the underlying data. `AdminReportController` keeps its two-type dispatch for resignation and salary until they also get their own bundles.
