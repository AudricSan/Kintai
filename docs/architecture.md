...
## 🧩 Modular Bundles
Features are split into **Core** (always on) and **Bundles** (`src/Bundles/*/`) — both are part of this AGPL-3.0 repository, nothing is gated behind a separate license. Any feature beyond the always-on core is expected to ship as a bundle: today that's Messaging, Daily Reports, Store Photos, and Timeclock. Time off, shift swaps, open shifts (shift claims), and the advanced HR reports (hiring, resignation, payroll) currently live in Core rather than as bundles — they are candidates for a future migration into their own bundles. Timeclock follows the same "UI detachable, data stays Core" pattern as Time Off/Shift Swap: `TimeclockRepositoryInterface` stays registered by the Core `RepositoryServiceProvider` because `HomeController` (admin dashboard "who's clocked in now" widget) and `EmployeeController::dashboard()` (employee's own "active clock" widget) both read it for calculations that must keep working even when the bundle is disabled. Disabling "timeclock" removes the clock-in/out UI (employee page, admin entries list/edit/delete, `/api/v1/timeclocks`) but not those two dashboard widgets or any already-recorded data. The existing per-store `timeclock` feature toggle now also gates on this bundle via `AdminStoreController::FEATURE_BUNDLE_MAP`, same as `messages`/`daily_reports`/`photos`.
