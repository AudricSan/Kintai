...
## 🧩 Modular Bundles
Features are split into **Core** (always on) and **Bundles** (`src/Bundles/*/`) — both are part of this AGPL-3.0 repository, nothing is gated behind a separate license. Any feature beyond the always-on core is expected to ship as a bundle: today that's Messaging, Daily Reports, Store Photos, and Shift Swaps. Time off, open shifts (shift claims), and the advanced HR reports (hiring, resignation, payroll) currently live in Core rather than as bundles — they are candidates for a future migration into their own bundles. The Shift Swap bundle is a partial exception: `ShiftSwapRequestRepositoryInterface` stays a Core service (`StoreStatsService`, `HomeController`, and the employee dashboard all read swap data for calculations that must keep working regardless), so disabling the bundle removes the swap management UI but not the underlying data or those Core calculations.
