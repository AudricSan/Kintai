...
- **La Action échoue à publier** : vérifier l'exécution du workflow `Release` dans l'onglet Actions ; la cause la plus fréquente est une version dans `composer.json` qui ne correspond encore à aucune section `## [...]` dans `CHANGELOG.md` (l'étape des notes de release retombe alors sur un texte de substitution, elle ne fait pas échouer le build). Il reste possible de taguer et publier manuellement avec `gh release create` si besoin.
