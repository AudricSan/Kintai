...
## 🧩 Bundles modulaires
Les fonctionnalités sont séparées en **Core** (toujours actif) et **Bundles** (`src/Bundles/*/`) — les deux font partie de ce dépôt AGPL-3.0, rien n'est verrouillé derrière une licence séparée. Toute fonctionnalité au-delà du cœur toujours actif est destinée à être livrée sous forme de bundle : aujourd'hui Messaging, Daily Reports, Store Photos et le rapport d'embauche. Les congés, échanges de shifts, bourse aux shifts (shift claims), et les rapports RH restants (démission, salaire) vivent aujourd'hui dans le Core plutôt qu'en bundles — ce sont des candidats à une future migration vers leurs propres bundles. Embauche, démission et salaire partageaient auparavant un seul `AdminReportController` avec un dispatch interne `repo(string $type)` ; l'extraction de l'embauche a déplacé la logique CRUD/PDF commune aux trois types de rapports dans un trait réutilisable `HasStaffReportCrud`, utilisé par `AdminHiringReportController` du bundle. Contrairement à démission/salaire, `HiringReportRepositoryInterface` reste un service Core — `AdminUserController` le lit/écrit directement pour générer automatiquement un rapport d'embauche à chaque création d'employé (formulaire standard et import Excel rapide), un mécanisme du cœur de la gestion des utilisateurs qui doit continuer de fonctionner même si ce bundle est désactivé. Désactiver "hiring-report" ne retire que l'UI de consultation/édition/PDF, pas cette génération automatique ni les données. `AdminReportController` conserve son dispatch à deux types pour démission et salaire tant qu'ils n'ont pas, eux aussi, leur propre bundle.
