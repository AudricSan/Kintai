...
- **Contrat stable :** `kintai\Core\BundleContract\Bundle` est la classe de base gelée, versionnée indépendamment, qu'un auteur de bundle doit étendre — un changement cassant à cet endroit est un événement documenté et annoncé au CHANGELOG, contrairement au reste de `src/Core/`, qui peut évoluer entre deux versions mineures sans préavis. `kintai\Core\Bundle` (l'ancien emplacement) est désormais un alias déprécié conservé uniquement pour que les 11 bundles du monorepo continuent de fonctionner sans changement le temps de leur migration. C'est la première étape d'un passage de bundles uniquement en monorepo vers des bundles versionnés et distribués séparément (manifeste + registry + installateur, en cours).
