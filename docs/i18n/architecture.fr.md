...
## 🧩 Bundles modulaires
Les fonctionnalités sont séparées en **Core** (toujours actif) et **Bundles** (`src/Bundles/*/`) — les deux font partie de ce dépôt AGPL-3.0, rien n'est verrouillé derrière une licence séparée. Toute fonctionnalité au-delà du cœur toujours actif est destinée à être livrée sous forme de bundle : aujourd'hui Messaging, Daily Reports, Store Photos et Pointage. Les congés, échanges de shifts, bourse aux shifts (shift claims), et les rapports RH avancés (embauche, démission, salaire) vivent aujourd'hui dans le Core plutôt qu'en bundles — ce sont des candidats à une future migration vers leurs propres bundles. Le bundle Pointage suit le même schéma « UI détachable, données dans le Core » que Congés/Échanges de shifts : `TimeclockRepositoryInterface` reste enregistré par le `RepositoryServiceProvider` du Core, car `HomeController` (widget « qui est pointé en ce moment » du dashboard admin) et `EmployeeController::dashboard()` (widget « pointage en cours » de l'employé) en dépendent tous deux pour des calculs qui doivent continuer de fonctionner même si le bundle est désactivé. Désactiver « timeclock » retire l'UI de pointage (page employé, liste/édition/suppression admin des entrées, `/api/v1/timeclocks`) mais pas ces deux widgets ni les données déjà enregistrées. Le toggle par store `timeclock` déjà existant se filtre désormais aussi sur ce bundle via `AdminStoreController::FEATURE_BUNDLE_MAP`, comme `messages`/`daily_reports`/`photos`.
