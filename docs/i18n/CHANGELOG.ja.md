...
### 変更
- `/docs` のハブページを分かりやすく改善しました：Kintai内でレンダリングされるローカルキャッシュ版を開くのか、GitHub.com上のライブページを開くのか、これまで判別しにくい状態でした。各言語カードにステータスバッジ（「このサーバーにキャッシュ済み」／「まだキャッシュされていません」）、それぞれ独自の説明文が付いた明確に区別される2つのアクション（塗りつぶしボタンの「Kintai内で読む」— ローカルコピー、オーナーが手動で更新 — とアウトラインボタンの「GitHubで見る」— 公式ソース、常に最新）、そしてグリッド上部に全体で使われている📖／🔗アイコンを説明する凡例を追加しました。
