...
## 🧩 モジュール型バンドル
機能は **Core**（常に有効）と **Bundles**（`src/Bundles/*/`）に分かれていますが、いずれもこのAGPL-3.0リポジトリの一部であり、別ライセンスの下に隠された機能はありません。常時有効なコアを超える機能はすべてバンドルとして提供される想定です。現在はMessaging、Daily Reports、Store Photos、退職報告書があります。休暇申請、シフト交換、オープンシフト（シフト応募）、採用報告書、給与報告書は現在Core内に存在しており、将来的に独自のバンドルへ移行する候補です。退職報告書と採用・給与報告書は以前、内部で `repo(string $type)` ディスパッチを行う単一の `AdminReportController` を共有していました。退職報告書の抽出により、3つのレポートタイプ共通のCRUD/PDFロジックが再利用可能なトレイト `HasStaffReportCrud` に移動し、バンドルの `AdminResignationReportController` で使用されています。給与報告書が独自のバンドルを持つ際には、採用・給与報告書がCoreでまだ共有している2タイプのディスパッチを置き換える準備ができています。
