...
## 🧩 モジュール型バンドル
機能は **Core**（常に有効）と **Bundles**（`src/Bundles/*/`）に分かれていますが、いずれもこのAGPL-3.0リポジトリの一部であり、別ライセンスの下に隠された機能はありません。常時有効なコアを超える機能はすべてバンドルとして提供される想定です。現在はMessaging、Daily Reports、Store Photos、そして打刻があります。休暇申請、シフト交換、オープンシフト（シフト応募）、および高度な人事レポート（採用、退職、給与）は現在Core内に存在しており、将来的に独自のバンドルへ移行する候補です。打刻バンドルは休暇申請/シフト交換と同じ「UIのみ分離、データはCoreに残す」方式に従います——`TimeclockRepositoryInterface`はCoreの`RepositoryServiceProvider`に登録されたままです。`HomeController`（管理者ダッシュボードの「現在打刻中」ウィジェット）と`EmployeeController::dashboard()`（従業員自身の「打刻中」ウィジェット）の両方がこれを読み取っており、バンドルが無効化されても動作し続ける必要があるためです。「timeclock」を無効化すると、打刻UI（従業員ページ、管理者側の一覧・編集・削除、`/api/v1/timeclocks`）は失われますが、この2つのウィジェットや既に記録されたデータには影響しません。既存の店舗単位の`timeclock`機能トグルは、`messages`/`daily_reports`/`photos`と同様に、`AdminStoreController::FEATURE_BUNDLE_MAP`経由でこのバンドルによってもフィルタリングされるようになりました。
