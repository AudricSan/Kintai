...
## 🧩 モジュール型バンドル
機能は **Core**（常に有効）と **Bundles**（`src/Bundles/*/`）に分かれていますが、いずれもこのAGPL-3.0リポジトリの一部であり、別ライセンスの下に隠された機能はありません。常時有効なコアを超える機能はすべてバンドルとして提供される想定です。現在はMessaging、Daily Reports、Store Photos、そして給与報告書があります。休暇申請、シフト交換、オープンシフト（シフト応募）、および残りの人事レポート（採用、退職）は現在Core内に存在しており、将来的に独自のバンドルへ移行する候補です。採用・退職・給与はかつて単一の`AdminReportController`を共有し、内部の`repo(string $type)`ディスパッチで処理されていました。給与の抽出により、3つのレポート種別に共通するCRUD/PDFロジックは再利用可能な`HasStaffReportCrud`トレイトへ移され、バンドルの`AdminSalaryReportController`で使用されています。その`calculateSalaryPreset()`メソッドは、当月の売上合計を事前入力するために`DailyReportRepositoryInterface`（DailyReportバンドル）を読み取ります——バンドル間依存であり、給与報告書の作成にはdaily-reportが有効なままであることが前提です。`AdminReportController`は、退職も独自のバンドルを持つまでは採用・退職の2種類ディスパッチを維持し、その時点で採用単独が`HasStaffReportCrud`へ移行する予定です。
